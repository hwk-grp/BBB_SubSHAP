...
def train(model, data_loader, criterion, optimizer, scheduler):
    ...
    for i, batch in enumerate(data_loader):
        ...
        train_y = batch[:, -1].clone().detach().float().view(-1, 1)
    ...
    preds_list_all_np = (preds_list_all >= 0.5)
    ...
    train_p_auc = roc_auc_score(targets_list_all, preds_list_all)
...
def test(model, data_loader, criterion):
    ...
    with torch.no_grad():
        ...
        test_p_auc = roc_auc_score(targets_list_all, preds_list_all)
