...
train_targets = numpy.array(targets[:num_train_mols]).reshape(-1, 1)
...
train_dataset = numpy.hstack([train_dataset, train_targets]).astype(numpy.float32)
valid_dataset = numpy.hstack([valid_dataset, valid_targets]).astype(numpy.float32)
test_dataset = numpy.hstack([test_dataset, test_targets]).astype(numpy.float32)
...
for epoch in range(max_epochs):
    ...
    # Now train/test return sensitivity, specificity, and fpr as well
    (train_loss, train_acc, train_f1, train_auc, train_p_auc,
     train_sens, train_spec, train_fpr) = tr_dnn.train(model, train_data_loader, criterion, optimizer, scheduler)

    (valid_loss, valid_acc, valid_f1, valid_auc, valid_p_auc,
     valid_sens, valid_spec, valid_fpr) = tr_dnn.test(model, valid_data_loader, criterion)

    (test_loss, test_acc, test_f1, test_auc, test_p_auc,
     test_sens, test_spec, test_fpr) = tr_dnn.test(model, test_data_loader, criterion)

    # Early stopping based on valid pAUC
    if valid_p_auc > best_score:
        best_score, best_epoch = valid_p_auc, epoch
        save_best_checkpoint(model, optimizer, best_epoch, best_score, rand_seed, architecture)
        early_stop_cnt = 0
    else:
        early_stop_cnt += 1
    ...
    print(f"Epoch {epoch+1} | Train loss : {train_loss:.4f} | Valid loss : {valid_loss:.4f} | Test loss : {test_loss:.4f} | Valid p-AUC : {valid_p_auc:.4f} | Test p-AUC : {test_p_auc:.4f}")

# ======================================
# Load the best model and evaluate
# ======================================
model_best = load_best_result(model, rand_seed, architecture)

(train_loss, train_acc, train_f1, train_auc, train_p_auc,
 train_sens, train_spec, train_fpr) = tr_dnn.test(model_best, train_data_loader, criterion)

(valid_loss, valid_acc, valid_f1, valid_auc, valid_p_auc,
 valid_sens, valid_spec, valid_fpr) = tr_dnn.test(model_best, valid_data_loader, criterion)

(test_loss, test_acc, test_f1, test_auc, test_p_auc,
 test_sens, test_spec, test_fpr) = tr_dnn.test(model_best, test_data_loader, criterion)

# ======================================
# Print all metrics
# ======================================
print(f"\n=== [{feature_type}] FINAL RESULTS ===")
...
torch.save(model.state_dict(),
           f'preds/saved_models/XAI_model_{dataset_name}_seed_{rand_seed}_{feature_type}.pt')
